...
use crate::instrs::{CompProg, Slot};
use crate::rules::{apply_rule, make_rule, Op, Rule};
use crate::tape::{BasicTape, EnumTape, GetSig, Signature};
...
pub struct Prover {
    ...
    rules: HashMap<Slot, Vec<(MinSig, Rule)>>,
...
impl Prover {
    ...
    pub fn try_rule(
        ...
        {
            let past_configs = self.configs.get_mut(&sig).unwrap();
